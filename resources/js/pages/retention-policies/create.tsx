...
export function CreateRetentionPolicyDialog({ isOpen, onClose }: CreateRetentionPolicyDialogProps) {
    const { data, setData, post, reset, errors, processing } = useForm<RetentionPolicyCreateRequest>({
        code: '',
        name: '',
        description: '',
    ...
    const onSubmit: FormEventHandler = (e) => {
        e.preventDefault();
        post(route('retention-policies.store'), {
                ...
                <form onSubmit={onSubmit}>
                    <DialogHeader className="mb-4">
                        ...
                        <DialogDescription className="text-xs">
                            Add a new retention policy by providing the required details. This will be included in the system records.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <Field>
                            <Label htmlFor="code">Code</Label>
                            <Input name="code" placeholder="e.g. SHORT_TERM" onChange={onChangeInput} />
                            <span className="text-orange-600">{errors.code}</span>
                        </Field>
                        <Field>
                            <Label htmlFor="name">Name</Label>
                            <Input name="name" placeholder="e.g. Short Term" onChange={onChangeInput} />
                            <span className="text-orange-600">{errors.name}</span>
                        </Field>
                        <Field>
                            <Label htmlFor="description">Description</Label>
                            <Input name="description" placeholder="e.g. Documents retained for 6 months" onChange={onChangeInput} />
                            <span className="text-orange-600">{errors.description}</span>
                        </Field>
                        <Field>
                            <Label htmlFor="archive_after_months">Archive After (Months)</Label>
                            <Input name="archive_after_months" type="number" placeholder="e.g. 6" onChange={onChangeInput} />
                            <span className="text-orange-600">{errors.archive_after_months}</span>
                        </Field>
                        <Field>
                            <Label htmlFor="delete_after_years">Delete After (Years)</Label>
                            <Input name="delete_after_years" type="number" placeholder="e.g. 1" onChange={onChangeInput} />
